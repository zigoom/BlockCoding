...
namespace GVRMUI
{
    public class Tuner : MUI
    {
        ...
        public override void Press(Transform controllerTransform)
        {
            Quaternion delta = Quaternion.Inverse(lastRotation) * controllerTransform.rotation;
            ...
            else if (relRot > detentStep)
            { //check wether the detent is passed
                if (relRot < 180)
                {
